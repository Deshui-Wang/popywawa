<template>
  <div class="list-page">
    ...
    <section class="toy-grid">
      <div 
        v-for="(toy, index) in seriesConfig.toys" 
        :key="toy.name + index" 
...
const seriesData = {
  ...
  'wtz-all': { name: '问童子', fullName: '问童子全系列陈列', toys: [{ name: '问童子 01', image: '/pic/wentongzi/wtz01.jgp' }, { name: '问童子 02', image: '/pic/wentongzi/wtz02.jgp' }, { name: '问童子 03', image: '/pic/wentongzi/wtz03.jgp' }, { name: '问童子 04', image: '/pic/wentongzi/wtz04.jgp' }, { name: '问童子 05', image: '/pic/wentongzi/wtz05.jgp' }] },
...
const triggerAuth = () => {
  console.log('Action triggered for:', seriesId.value);
  showAuth.value = true
}
...
const goToDetail = (toy) => {
  router.push({
    ...
    query: { name: toy.name, image: toy.image }
  })
}
...
const handleAuthSuccess = () => {
  router.push({
    name: 'publish',
    query: { brand: route.params.brandId, series: seriesConfig.value.name }
  })
}
</script>
